...
check "substitute key file is written" sh -c 'find /etc/guix/acl -maxdepth 1 -name "*.pub" | grep -q .'
check "substitute key content is decoded" sh -c 'grep -q "fake-nonguix-signing-key" /etc/guix/acl/*.pub'
